<template>
  <div class="creermatchingpage bg-slate-950 text-white p-4 lg:p-6">
    ...
    >
      ...
      >
        <div class="w-full xs:w-auto md:flex-1">
          ...
          >
            Career Matching Questionnaire 2
          </h1>
          <p
        ...
        <div class="flex gap-2 sm:gap-3 w-full xs:w-auto">
          <button
            @click="saveProgress"
            class="flex-1 xs:flex-none bg-slate-800 hover:bg-slate-700 px-4 sm:px-6 py-1.5 sm:py-2 rounded-lg text-xs sm:text-sm transition-colors focus:outline-none focus:ring-2 focus:ring-blue-500/50 border border-slate-700"
          >
            Save Progress
          </button>
          <button
            @click="submitQuestionnaire"
            class="flex-1 xs:flex-none bg-blue-600 hover:bg-blue-500 px-4 sm:px-6 py-1.5 sm:py-2 rounded-lg text-xs sm:text-sm transition-colors focus:outline-none focus:ring-2 focus:ring-blue-500/50 font-medium"
          >
            Submit
          </button>
        </div>
    ...
    <main class="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div class="space-y-4">
          <div class="space-y-2">
            <label class="block text-sm font-medium text-slate-300"
              >What motivates you the most?</label
            >
            <select
              v-model="personalData.motivation"
              class="w-full bg-slate-800 rounded-lg px-4 py-2.5 text-sm border border-slate-700 hover:border-slate-600 transition-colors focus:outline-none focus:ring-2 focus:ring-blue-500/50"
            >
          ...
          <div class="space-y-2">
            <label class="block text-sm font-medium text-slate-300"
              >How do you prefer to work?</label
            >
          ...
          <div class="space-y-2">
            <label class="block text-sm font-medium text-slate-300"
              >Rate your stress management skills</label
            >
            <div class="flex items-center gap-4">
              ...
              />
              <span class="text-sm text-slate-400 min-w-[2.5rem]"
                >{{ personalData.stressManagement }}/5</span
              >
            </div>
          </div>
        </div>
      ...
      <div class="bg-slate-900/50 backdrop-blur-sm p-5 rounded-xl shadow-lg">
        <div class="flex items-start gap-4 mb-6">
          ...
          <div>
            ...
            <p class="text-sm text-slate-400">
              Tell us about your abilities and passions
            </p>
          </div>
        </div>
        ...
        <div class="space-y-4">
          <div class="space-y-2">
            <label class="block text-sm font-medium text-slate-300"
              >Select your top skills (up to 5)</label
            >
          ...
          <div class="space-y-2">
            <label class="block text-sm font-medium text-slate-300"
              >What are your career interests?</label
            >
            <textarea
              v-model="personalData.careerInterests"
              rows="3"
              class="w-full bg-slate-800 rounded-lg px-4 py-2.5 text-sm border border-slate-700 hover:border-slate-600 transition-colors focus:outline-none focus:ring-2 focus:ring-blue-500/50 resize-none"
      ...
      <div class="bg-slate-900/50 backdrop-blur-sm p-5 rounded-xl shadow-lg">
        ...
        <div class="space-y-4">
          ...
          <div class="space-y-2">
            <label class="block text-sm font-medium text-slate-300"
              >Salary Expectations (Annual)</label
            >
            <select
    ...
    <div class="flex justify-end gap-4 mt-6">
      <button
        @click="resetForm"
        class="px-6 py-2.5 bg-slate-800 hover:bg-slate-700 rounded-lg text-sm font-medium transition-colors focus:outline-none focus:ring-2 focus:ring-blue-500/50"
      >
        Reset
      </button>
      <button
        @click="submitQuestionnaire"
        class="px-6 py-2.5 bg-blue-600 hover:bg-blue-500 rounded-lg text-sm font-medium transition-colors focus:outline-none focus:ring-2 focus:ring-blue-500/50"
      >
...
export default {
  setup() {
    ...
    const store = useCareerMatchingStore();
    const { personalData, educationData, careerData, progress } =
      storeToRefs(store);
    return {
      store,
      personalData,
  ...
  methods: {
    async saveProgress() {
      try {
        ...
        if (!userId) {
          this.$toast.error("Please sign in to save your progress");
          return;
        }

        await this.store.saveProgress(userId);
    ...
    async submitQuestionnaire() {
      if (this.store.progress < 100) {
        this.toast.warning("Please complete all sections before submitting.");
        return;
      }
      ...
      try {
        ...
        if (!userId) {
          this.toast.error("Please sign in to submit the questionnaire");
          return;
        }
        ...
        const submissionId = await this.store.saveResultsToDb(submissionData);
    ...
    async loadProgress() {
      try {
        ...
        if (userId) {
          await this.store.loadProgress(userId);
        } else {
    ...
    loadFromLocalStorage() {
      ...
      if (savedData) {
        ...
        Object.assign(this.personalData, parsedData.personalData);
        Object.assign(this.educationData, parsedData.educationData);
        Object.assign(this.careerData, parsedData.careerData);
      }
    },
    ...
    toggleSkill(skill) {
      if (this.personalData.selectedSkills.includes(skill)) {
        this.personalData.selectedSkills =
          this.personalData.selectedSkills.filter((s) => s !== skill);
      } else if (this.personalData.selectedSkills.length < 5) {
        this.personalData.selectedSkills.push(skill);
      }
      this.store.calculateProgress();
    },

    toggleWorkEnvironment(env) {
      if (this.careerData.preferredEnvironments.includes(env)) {
        this.careerData.preferredEnvironments =
          this.careerData.preferredEnvironments.filter((e) => e !== env);
      } else {
    ...
    isSkillSelectionDisabled(skill) {
      return (
        !this.personalData.selectedSkills.includes(skill) &&
